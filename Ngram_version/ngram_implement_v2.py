...
def chinese_eng_break(resource, chinese_path, english_path):
            ...
            for text in range(0, len(content)):
                ...
                if content[text] == ' ':
                    if content[text - 1] == ' ':
                        ...
                        eng += " "
                    elif '\u4e00' <= content[text-1] <= '\u9fcb':
                        chi += " "
                    else:
                        pass
                elif '\u0e20' <= content[text] <= '\u002f':
                    if '\u0041' <= content[text - 1] <= '\u005a' or '\u0061' <= content[text - 1] <= '\u007a':
                        eng += " "
                    elif '\u4e00' <= content[text-1] <= '\u9fcb':
                        chi += " "
                    else:
                    ...
                    if '\u0041' <= content[text - 1] <= '\u005a' or '\u0061' <= content[text - 1] <= '\u007a':
                        ...
                        pass
                elif '\u0041' <= content[text] <= '\u005a' or '\u0061' <= content[text] <= '\u007a':  # 符號編碼ex: ":,.#$%&/" 20-2f  # 數字編碼 30~39  # 大小寫英文與符號編碼 大寫41-5a 小寫61-7a
                    eng += content[text]
    ...
    with open(chinese_path, "w+", encoding="utf-8-sig") as file1:
        file1.write(chi)
    with open(english_path, "w+", encoding="utf-8-sig") as file2:
        file2.write(eng.lower())
